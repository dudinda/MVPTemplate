...
## How to Test
   ...
   7. Create a base test, use the static state machine to Build partial substitutes on the <code>[SetUp]</code> and to Exit on the <code>[TearDown]</code>.
   8. You can now write asynchronous scenario using Mocks. Raise an event from a form wrapper, when use <code>.Receieved()</code> interface to validate arguments and returns.
